...
func request_market_info(p_coords: Vector2i) -> void:
	...
	if town != null:
		set_market_info.rpc_id(multiplayer.get_remote_sender_id(), town.get_current_hold())
	set_market_info.rpc_id(multiplayer.get_remote_sender_id(), {})


# === Recieves ===
@rpc("authority", "call_local", "unreliable")
func set_market_info(info: Dictionary) -> void:
	if info.is_empty():
		return
	var term_singleton: terminal_map = terminal_map.get_instance()
	for index: int in range(0, info.size()):
		var text: String = term_singleton.get_cargo_name(index) + ": " +str(info[index])
