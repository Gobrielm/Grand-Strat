...
static func is_tile_open(coords: Vector2i, id: int) -> bool:
	var val: bool = !Utils.is_tile_taken(coords) and rail_placer.get_instance().get_track_connection_count(coords) == 0 and tile_ownership.get_instance().is_owned(id, coords)
	return val
...
static func just_has_rails(coords: Vector2i, id: int) -> bool:
		return !Utils.is_tile_taken(coords) and tile_ownership.get_instance().is_owned(id, coords)
